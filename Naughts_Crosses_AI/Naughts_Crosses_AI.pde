...
int computer() 
{
    if (AILevel == 1) {
      ...
      int ranSquare = int(random(0,count));
...
void drawCross(float x, float y, float r, float thickness) {
...
void drawNaught(float x, float y, float r, float thickness) {
  float cx = leftMargin + (x*boxWidth) + boxWidth/2;
  float cy = topMargin + (y*boxWidth) + boxWidth/2;
  strokeWeight(thickness);
  noFill();
...
int Mouse2X() {
  int X;
  X = int((mouseX - leftMargin)/boxWidth);
  return X;
}

int Mouse2Y() {
  int Y;
  Y = int((mouseY - topMargin)/boxWidth);
  return Y;
}

int checkBoard() { // 0=no winner 1=crosses 2=naughts 3=draw
  stroke(0);
  strokeWeight(15);
  //checking crosses
  //columns
